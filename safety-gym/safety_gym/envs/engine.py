...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    def step_simulate(self, ind_main_action, action_list):
        ''' Simulate the effect of multiple action but return
            only the effect of the action at index ind_main_action
        '''
        assert not self.done, 'Environment must be reset before stepping'
        # Save the current state
        curr_state = self.sim.get_state()
        if self.action_noise:
            noise_to_add = self.action_noise * self.rs.randn(self.model.nu)
        nb_iter_simu = self.rs.binomial(self.frameskip_binom_n,
                                                self.frameskip_binom_p)
        # Get action range
        action_range = self.model.actuator_ctrlrange
        # Resulting reward and cost
        res_r_c = np.zeros((2, len(action_list)))
        # Simulate the non-essential action
        mainAction = None
        for ind_act, action_val in enumerate(action_list):
            if ind_act == ind_main_action:
                mainAction = action_val
                continue
            info = {}
            self.sim.set_state(curr_state)
            action = np.array(action_val, copy=False) # cast to ndarray
            self.data.ctrl[:] = np.clip(action, action_range[:,0],
                                            action_range[:,1])
            # Add noise if enabled
            if self.action_noise:
                self.data.ctrl[:] += noise_to_add
            # Simulate physics forward
            exception = False
            for _ in range(nb_iter_simu):
                try:
                    self.set_mocaps()
                    self.sim.step()
            ...
            if exception:
                ...
                self.sim.forward()
                reward = self.reward(simulate=True)
        ...
        info = {}
        self.sim.set_state(curr_state)
        action = np.array(mainAction, copy=False) # cast to ndarray
        self.data.ctrl[:] = np.clip(action, action_range[:,0],
                                            action_range[:,1])
        if self.action_noise:
            self.data.ctrl[:] += noise_to_add
        ...
        if exception:
            ...
            self.sim.forward()
            reward = self.reward(simulate=False)
            info.update(self.cost())
        ...
        return self.obs(), reward, self.done, info, res_r_c
    ...
    def reward(self, simulate=False):
